...
const {dueAmount, depositAmount, depositDuDate} = storeToRefs(store);

const {editDepositRequest} = store;


const toast = useToast();

let percentage = ref(0);
let amount = ref(depositAmount.value);
let dueDate = ref(new Date(depositDuDate.value));
...
const submitDepositModal = () => {
    ...
    editDepositRequest(amount.value,dueDate.value)
